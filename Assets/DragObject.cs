		...
		foreach (Touch touch in Input.touches) {
			...
			if (touch.phase == TouchPhase.Began) {
				...
				if (Physics.Raycast(ray, out hit, 1000)) { //True when Ray intersects colider. 
					...
					animal_7 = pickedObject.GetComponent<GameObject>(); 
					lastPlanePoint = planePoint;
					animal_1.SetActive(false); 
					animal_2.SetActive(false); 
					animal_3.SetActive(false);
					animal_4.SetActive(false); 
					animal_5.SetActive(false); 
					animal_6.SetActive(false);
				} else {
					pickedObject = null;
				}
